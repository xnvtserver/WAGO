<template>
  <div>
    ...
    <div v-if="!currentUser?.shop_id" class="bg-orange-50 border-l-4 border-orange-400 p-4 mb-6">
      <div class="flex items-center">
        <i class="fas fa-exclamation-triangle text-orange-500 mr-3"></i>
        <p class="text-sm text-orange-700">
          വില മാറ്റാൻ നിങ്ങൾ ലോഗിൻ ചെയ്തിരിക്കണം!
        </p>
      </div>
    </div>

    <!-- Price Inputs (only show when authenticated) -->
    <div v-if="currentUser?.shop_id">
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
          ...
          <div class="flex items-center">
            ...
            <input 
              ...
              v-model.number="store.prices.purchase"
            >
        ...
        <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
          ...
          <div class="flex items-center">
            ...
            <input 
              ...
              v-model.number="store.prices.retail"
            >
            <span class="ml-2 text-blue-500">/യൂണിറ്റ്</span>
          </div>
          <div class="flex justify-between mt-2">
            <p class="text-xs text-blue-600">ലാഭം: ₹{{ validRetailProfit.toFixed(2) }}</p>
            <p class="text-xs text-blue-600">ലാഭ %: {{ validRetailProfitPercentage }}%</p>
          </div>
        </div>
        <!-- Wholesale Price -->
        <div class="bg-green-50 p-4 rounded-lg border border-green-100">
          ...
          <div class="flex items-center">
            ...
            <input 
              ...
              v-model.number="store.prices.wholesale"
            >
            <span class="ml-2 text-green-500">/യൂണിറ്റ്</span>
          </div>
          <div class="flex justify-between mt-2">
            <p class="text-xs text-green-600">ലാഭം: ₹{{ validWholesaleProfit.toFixed(2) }}</p>
            <p class="text-xs text-green-600">ലാഭ %: {{ validWholesaleProfitPercentage }}%</p>
          </div>
        </div>
      </div>
...
// Safe computed properties to handle NaN values
const validRetailProfit = computed(() => {
  return isNaN(store.retailProfit) ? 0 : store.retailProfit;
});

const validRetailProfitPercentage = computed(() => {
  return isNaN(store.retailProfitPercentage) ? 0 : store.retailProfitPercentage;
});

const validWholesaleProfit = computed(() => {
  return isNaN(store.wholesaleProfit) ? 0 : store.wholesaleProfit;
});

const validWholesaleProfitPercentage = computed(() => {
  return isNaN(store.wholesaleProfitPercentage) ? 0 : store.wholesaleProfitPercentage;
});
...
const debouncedSave = debounce(async () => {
  try {
    ...
    // Validate prices - check for NaN, null, undefined, and negative values
    const prices = store.prices;
    if (
      isNaN(prices.purchase) || 
      isNaN(prices.retail) || 
      isNaN(prices.wholesale) ||
      ...
      // saveStatus.value = 'error';
      // saveMessage.value = 'ദയവായി എല്ലാ വിലകളും സാധുതയുള്ള പോസിറ്റീവ് സംഖ്യകളാക്കി നൽകുക!';
      return;
    }
    ...
    saveStatus.value = 'error';
    saveMessage.value = error.message || 'വില മാറ്റം സേവ് ചെയ്യുന്നതിൽ പിശക്!';
  } finally {
    setTimeout(() => {
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    // Only trigger save if values actually changed
    if (
      newVal.purchase !== oldVal.purchase || 
