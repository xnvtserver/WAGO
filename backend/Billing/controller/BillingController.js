          ...
          if (billCustomer && (billCustomer.name || billCustomer.phone || billCustomer.address)) {
            const newCustomer = await transaction('customers')
              .insert({
                ...
                address: billCustomer.address ? JSON.stringify(billCustomer.address) : null,
            ...
            // Handle PostgreSQL return format
            if (Array.isArray(newCustomer)) {
              const inserted = newCustomer[0];
              customer_id = typeof inserted === 'object' ? inserted.id : inserted;
            } else {
